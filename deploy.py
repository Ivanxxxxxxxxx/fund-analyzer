...
for f in FRONT_FILES:
  push_file(FRONT_REPO, f, f"fix: 固定尺寸按钮padding归零(to-top/pk-slot/hc-del)+手机公共牌等宽 ({f})")

print("\n[2/2] 后端源码 → 仓库 %s" % BACK_REPO)
for f in BACK_FILES:
  push_file(BACK_REPO, f, f"fix: 固定尺寸按钮padding归零(to-top/pk-slot/hc-del)+手机公共牌等宽 ({f})")
